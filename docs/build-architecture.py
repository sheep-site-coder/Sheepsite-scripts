...
<table>
  ...
  <tr><td><code>my-unit.php</code> <span class="tag tag-php">PHP</span></td>
      <td>Shows the resident&rsquo;s unit information from the database. Provides a &ldquo;Request a Change&rdquo; form to notify the board by email.</td></tr>
  ...
  <tr><td><code>forgot-password.php</code> <span class="tag tag-php">PHP</span></td>
      <td>Self-serve password reset. Owner flow: username lookup via GAS, email temp password, update credentials. Admin flow: validates President&rsquo;s unit number before sending. <code>?setup=1</code> bootstraps first-time admin credentials. Owner reset is blocked when <code>testSite</code> is set; admin reset (<code>?role=admin</code>) is unaffected.</td></tr>
</table>
