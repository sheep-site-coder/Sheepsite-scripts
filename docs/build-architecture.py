...
<h3>Sheets Web App: building-script.gs (one per building) &mdash; Legacy / Non-DB Buildings</h3>
<div class="warn"><strong>DB-mode buildings:</strong> For buildings with MySQL enabled (<code>$useLocalDB = true</code>),
none of the Sheets web app endpoints below are called by PHP. Reports, owner import, and password reset
emails are all handled by PHP + MySQL. The <code>runScheduledUpdate</code> trigger still fires and
regenerates Sheet list tabs, but those tabs are no longer the data source for anything. The trigger
can be removed for DB-mode buildings.</div>
...
<table>
  <tr><th>URL / page param</th><th>What it does</th><th>DB-mode status</th></tr>
  <tr><td><code>.../exec</code></td><td>Board of Directors report</td><td>Dead &mdash; PHP serves from MySQL</td></tr>
  <tr><td><code>?page=elevator</code></td><td>Elevator List report</td><td>Dead &mdash; PHP serves from MySQL</td></tr>
  <tr><td><code>?page=parking</code></td><td>Parking List report</td><td>Dead &mdash; PHP serves from MySQL</td></tr>
  <tr><td><code>?page=resident</code></td><td>Resident List report</td><td>Dead &mdash; PHP serves from MySQL</td></tr>
  <tr><td><code>?page=owners&amp;token=...</code></td><td>Owner list JSON for import into manage-users.php</td><td>Dead &mdash; manage-users.php reads from MySQL</td></tr>
</table>
...
runs all four generate functions after a 30-second debounce). For non-DB buildings, this keeps
reports fresh within ~90 seconds of any data change. For DB-mode buildings, the Sheet is used
for the initial data import only and the triggers can be disabled.</p>
...
<h3>Overview</h3>
<p>Buildings with MySQL enabled (<code>$useLocalDB = true</code>) store all resident data in a
shared MySQL database on sheepsite.com. PHP scripts read and write the database directly &mdash;
no Google Sheets or Apps Script involved. The Google Sheet is used only for the initial one-time
import; after that, the database is the single source of truth.</p>
<p>The <code>$useLocalDB</code> flag is set to <code>true</code> automatically when both
<code>db/db.php</code> and <code>credentials/db.json</code> are present on the server. When false,
the system falls back to the legacy GAS/Sheet path for the affected operations.</p>
...
<table>
  ...
  <tr><td><code>db/residents.php</code></td><td>Full CRUD for residents, cars, unit_info, and emergency contacts. Field-name mapping between Sheet column names and MySQL column names. <code>sendTempPasswordEmail()</code> helper (PHP <code>mail()</code> from noreply@sheepsite.com). <code>dbGetEmailByUsername()</code> for username &rarr; email lookup (used by forgot-password.php).</td></tr>
...
<h3>What Changed vs. Legacy (GAS/Sheet) Path</h3>
<table>
  <tr><th>Feature</th><th>Legacy (GAS/Sheet)</th><th>DB-mode (MySQL)</th></tr>
  <tr><td>Reports (elevator, parking, resident, board)</td><td>GAS web app generates HTML from Sheet tabs</td><td>PHP queries MySQL directly in protected-report.php / public-report.php</td></tr>
  <tr><td>Admin reset &mdash; President lookup</td><td>GAS looks up President row in Sheet</td><td>PHP queries <code>residents</code> table where <code>board_role = &lsquo;President&rsquo;</code></td></tr>
  <tr><td>Owner import in manage-users.php</td><td>Calls <code>?page=owners&amp;token=...</code> GAS endpoint</td><td>Queries MySQL directly</td></tr>
  <tr><td>Resident change requests (my-unit.php)</td><td>Email sent to building contact; admin copies data manually</td><td>Request inserted into <code>res_requests</code>; admin accepts/rejects in database-admin.php</td></tr>
  <tr><td>Password reset email</td><td>GAS sends email via Gmail API</td><td>PHP <code>mail()</code> from noreply@sheepsite.com via <code>sendTempPasswordEmail()</code></td></tr>
  <tr><td>Resident data editing</td><td>Board edits Google Sheet directly</td><td>Admin uses database-admin.php on the website</td></tr>
</table>
...
<table>
  ...
  <tr><td><code>protected-report.php</code> <span class="tag tag-php">PHP</span></td>
      <td>Resident-authenticated reports (elevator, parking, resident). Reuses <code>private_auth_{{building}}</code> session. <strong>DB-mode:</strong> queries MySQL directly and renders inline PHP/HTML. <strong>Legacy:</strong> renders report HTML from the building&rsquo;s Sheets web app as an iframe.</td></tr>
  <tr><td><code>public-report.php</code> <span class="tag tag-php">PHP</span></td>
      <td>Unauthenticated reports (board of directors). Same structure as protected-report.php minus auth. <code>?nav=0</code> hides the back bar when embedded as an iframe. <strong>DB-mode:</strong> renders from MySQL inline; no iframe needed.</td></tr>
  ...
  <tr><td><code>my-unit.php</code> <span class="tag tag-php">PHP</span></td>
      <td>Shows the resident&rsquo;s unit information from the database. &ldquo;Request a Change&rdquo; form inserts a row into <code>res_requests</code> (DB-mode) or emails the board (legacy). Pending requests are reviewed in database-admin.php.</td></tr>
  ...
  <tr><td><code>forgot-password.php</code> <span class="tag tag-php">PHP</span></td>
      <td>Self-serve password reset. Owner flow: username lookup, email temp password, update credentials. Admin flow: validates President&rsquo;s unit number before sending. <strong>DB-mode:</strong> President&rsquo;s email and unit looked up from MySQL (<code>board_role = &lsquo;President&rsquo;</code>). <strong>Legacy:</strong> calls GAS to look up the President row from the Sheet. <code>?setup=1</code> bootstraps first-time admin credentials. Owner reset blocked when <code>testSite</code> is set.</td></tr>
</table>
...
<table>
  ...
  <tr><td><code>manage-users.php</code> <span class="tag tag-php">PHP</span></td>
      <td>Add, remove, and reset passwords for resident accounts. <strong>DB-mode:</strong> import reads from MySQL; password reset emails sent via <code>sendTempPasswordEmail()</code>. <strong>Legacy:</strong> import calls GAS <code>?page=owners</code> endpoint. Supports both building admin and master admin login.</td></tr>
  ...
  <tr><td><code>master-admin.php</code> <span class="tag tag-php">PHP</span></td>
      <td>SheepSite operator dashboard. Per-building cards showing Woolsy credits, storage (with last-updated age and ↺ per-card refresh button), ToS status, renewal date, and SUSPENDED/RENEWAL DUE badges. The <code>?action=refreshStorage&amp;building=X</code> AJAX handler calls dir-display-bridge.gs storageReport and saves the result to <code>config/{{building}}.json</code> without a page reload. System Tools: Woolsy Overview, License Agreements, Pricing, Architecture Manual, Add New Association, Remove Association.</td></tr>
...
<table>
  ...
  <tr><td><code>buildings.php</code> <span class="tag tag-php">PHP</span></td>
      <td><strong>The only file to edit when adding a building.</strong> Returns an associative array keyed by building name. Each entry: <code>publicFolderId</code>, <code>privateFolderId</code>, <code>webAppURL</code> (Sheets web app URL &mdash; still present but <strong>not called for reports in DB-mode</strong>; used only as a legacy fallback), <code>state</code> (two-letter state code for Woolsy FAQ layer), <code>community</code> (optional community name for shared FAQ layer, e.g. <code>CVE</code>).</td></tr>
...
<table>
  ...
  <tr><td><code>sheets/building-script.gs</code> <span class="tag tag-gs">GAS</span></td>
      ...
      <td>Report generation, owner import, password reset emails. <strong>DB-mode:</strong> no longer called by PHP &mdash; Sheet used for initial import only. Triggers can be disabled.</td></tr>
  <tr><td><code>sheets/board-list.gs</code> <span class="tag tag-gs">GAS</span></td>
      <td>DatabaseSheetMaster library</td>
      <td>Board of Directors report generator. <strong>Dead for DB-mode buildings</strong> &mdash; PHP renders from MySQL.</td></tr>
  <tr><td><code>sheets/elevator-list.gs</code> <span class="tag tag-gs">GAS</span></td>
      <td>DatabaseSheetMaster library</td>
      <td>Elevator List report. <strong>Dead for DB-mode buildings</strong> &mdash; PHP renders from MySQL.</td></tr>
  <tr><td><code>sheets/parking-list.gs</code> <span class="tag tag-gs">GAS</span></td>
      <td>DatabaseSheetMaster library</td>
      <td>Parking List report. <strong>Dead for DB-mode buildings</strong> &mdash; PHP renders from MySQL.</td></tr>
  <tr><td><code>sheets/resident-list.gs</code> <span class="tag tag-gs">GAS</span></td>
      <td>DatabaseSheetMaster library</td>
      <td>Resident List. <strong>Dead for DB-mode buildings</strong> &mdash; PHP renders from MySQL.</td></tr>
  <tr><td><code>sheets/owner-import.gs</code> <span class="tag tag-gs">GAS</span></td>
      <td>DatabaseSheetMaster library</td>
      <td>Returns Database tab as JSON for owner import into manage-users.php. <strong>Dead for DB-mode buildings</strong> &mdash; manage-users.php reads from MySQL.</td></tr>
  <tr><td><code>sheets/reset-password.gs</code> <span class="tag tag-gs">GAS</span></td>
      <td>DatabaseSheetMaster library</td>
      <td>Handles self-serve password reset email flow (owner and admin). <strong>Dead for DB-mode buildings</strong> &mdash; PHP sends email directly via <code>sendTempPasswordEmail()</code>.</td></tr>
</table>
